...
step7 <- penguins %>% 
  mutate(flippometers = flipper_length_mm / 1e6) #dividing flipper length mm by 1,000,000 to convert to km
...
count$n #display only n columns for count
```
...
final_boss <- penguins %>% #assigning to object final_boss
  filter(
    sex == "FEMALE", #filtering only female
    !is.na(flipper_length_mm), #removing na values
    island %in% c("Dream", "Biscoe") #filtering only islands dream and biscoe
  ) %>% 
  select(species, island, flipper_length_mm, body_mass_g) %>% #selecting specific columns
  ...
  group_by(species, island, flipper_rank) %>% #grouping by species, island, flip.rank 
  summarise( #summarizing body mass
    avg_mass_g = round(mean(body_mass_g, na.rm = TRUE), 0) #round to nearest # + dropping na
    )
