...
author: "Amber Camp (edited by Grace)"
date: "2025-09-08"
format: html
editor: visual
---
...
head(mtcars, 10) # displays the top few lines of the dataframe, adding comma to distinguish how many
tail(mtcars) #last few lines
...
```{r}
mean(mtcars$hp) # mean of gross horsepower
```

-   Can you find the median?

```{r}
median(mtcars$disp) # median of displacement (cu.in.)
```
...
ggplot(mtcars, aes(x = hp, y = disp)) +
  ...
  labs(
    ...
    subtitle = "Cars with more horsepower tend to have larger displacement (engine size).",
    x = "Horsepower",
