...
url_gentoo <- "https://pasta.lternet.edu/package/data/eml/knb-lter-pal/220/3/e03b43c924f226486f2f0ab6709d2381"
...
summary(penguins_data) #basic data exploration
head(penguins_data)
...
```{r, message=FALSE}
ggplot(data = penguins_data, aes(x = flipper_length_mm, fill = species)) +
  geom_histogram(alpha=0.5) + #alpha=transparency
  theme_bw() +
...
ggplot(data = penguins_data, aes(x = species, y = flipper_length_mm, fill = species)) +
  geom_boxplot() +
  #geom_point() + # just puts on one line
  geom_jitter(size = 0.5) + # adding some datapoints and jittering them, better for recognizing separate clusters
...
#as mass increases, flipper is longer
```
...
ggplot(data = penguins_data, aes(x = culmen_depth_mm, fill = species)) +
  theme_bw() +
  scale_fill_viridis_d() +
  geom_histogram() +
  labs(
    title = "Culmen Depth by species",
    subtitle = "part 1: histogram",
    x = "culmen depth (mm)",
    y = "count",
    fill = "species"
  )
...
ggplot(data = penguins_data, aes(x = culmen_depth_mm, y = body_mass_g, color = species)) +
  theme_bw() +
  scale_color_viridis_d(option = "mako") +
  geom_point() +
  labs(
    title = "Culmen Depth per Species",
    subtitle = "part 2: scatterplot",
    x = "culmen depth (mm)",
    y = "body mass (g)",
    color = "species"
  )
...
part3 <- ggplot(data = penguins_data, aes(x = culmen_depth_mm, y = body_mass_g, color = species)) +
  theme_bw() +
  scale_color_viridis_d(option = "mako") +
  geom_point() +
  labs(
