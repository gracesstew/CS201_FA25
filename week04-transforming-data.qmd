...
fs <- ggplot(data = penguins_male_female, aes(x = flipper_length_mm, y = body_mass_g, color = species, shape = island)) +
  geom_point() +
  theme_bw() +
  scale_color_viridis_d() +
  facet_wrap(~sex) +
...
2.  Now, create a bar graph using the *new* dataframe you made
    ...
    ```{r}
    ggplot(data = average_mass, aes(x = species, y = average_body_mass, fill = species)) + # fill in the blanks
      geom_bar(stat = "identity") +
      labs(
        title = "Average Mass By Species",
        x = "Species",
        y = "Body Mass (g)"
      ) +
      theme_minimal() +
...
ggplot(data = average_flipper, aes(x = species, y = average_flipper_length, fill = species)) +
  geom_bar(stat = "identity") +
  labs(
    title = "Average Flipper By Species",
    x = "Species",
    y = "Flipper Length (mm)"
  ) +
  theme_minimal() +
...
summary(asfactor.(penguins_data$island)) # $ selects variable of interest
...
average_culmen <- penguins_data %>% 
  group_by(species) %>% 
  summarise(average_culmen_depth = mean(culmen_depth_mm, na.rm = TRUE))

ggplot(data = average_culmen, aes(x = species, y = average_culmen_depth, fill = species)) +
  geom_bar(stat = "identity") +
  theme_bw() +
  theme(legend.position = "none") +
  labs(title = "Average Culmen Depth per Species", subtitle = "Exercise 4", x = "species", y = "culmen depth (mm)")
```
