...
**Answer:** Histograms are used for continuous data whereas bar charts are for categorical data.
...
-   Does your visualization match your expectations?
    ...
    -   Yes. I've played a lot of Pokemon games growing up and have noticed that water and normal types are common encounters.
...
spatk_bp <- ggplot(data = pokedata, aes(x = sp_atk, y = type_1, fill = type_1)) +
  geom_boxplot()
...
spdef_bp <- ggplot(data = pokedata, aes(x = sp_def, y = generation)) +
  geom_boxplot() +
...
-   How do you read a boxplot, and what are the different landmarks?
    -   The box itself represents the interquartile range. The bottom of the box represents the 25th percentile range and top is 75th percentile. The line within the box shows the median (50th percentile). If the median is closer to the bottom or top, this means the data is skewed. The whiskers that extend from the box are data points that are NOT outliers and the dots ARE outliers.
