...
ggplot(pokedata, aes(x = fct_reorder(type_1, hp, median), y = hp)) +
  geom_boxplot() +
  geom_point(alpha = 0.5) # try jitter, width of 0.2

ggplot(pokedata, aes(x = type_1, y = hp)) +
  geom_boxplot() +
  facet_wrap(~generation) + # try face_grid()
...
The factor with the most influence on price is \_\_\_\_. There may be a trade-off between `carat` and `cut`. Perhaps larger carat diamonds can be profitably sold even with a low quality cut, for example.
